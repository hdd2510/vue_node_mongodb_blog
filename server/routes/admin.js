...
//查找登录用户
router.get('/admin_information', function(req, res, next){
  var _id = req.query._id;
  console.log(req.query)
  Model.User.findOne({_id: _id}, function(err, doc){
    if(err){
      console.log(err);
      return;
    }else{
...
// 修改信息
router.post('/admin_change_information', function(req, res, next){
  var _id = req.body._id;
  var username = req.body.username;
  var newPassword = req.body.newPassword;
  var password = req.body.ComfirmPassword;
  var newInformation = {
    username: username,
    password: password,
  ...
  console.log(_id)
  Model.User.update({_id: _id}, {$set: newInformation}, function(err, doc){
    if(err){
      console.log(err);
      return;
    }else{
...
// 设置管理员
router.post('/admin_users_setAdmin', function(req, res, next){
  var _id = req.body._id;
  Model.User.update({_id: _id},{$set: {isAdmin : true}}, function(err, doc){
    if(err){
      console.log(err);
      return;
    }else{
      responseData.code = '200';
      responseData.message = '权限设置成功！';
      res.json(responseData);
      return;
    }
  })
...
//设置会员
router.post('/admin_users_setMember', function(req, res, next){
  var _id = req.body._id;
  Model.User.update({_id: _id},{$set: {isAdmin : false}}, function(err, doc){
    if(err){
      console.log(err);
      return;
    }else{
